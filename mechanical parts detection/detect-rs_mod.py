...
def detect(save_img=False):
    ...
    #     circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT,1,30,param1=60,param2=50,minRadius=0,maxRadius=0)
    #  #   circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT,1,30,param1=190,param2=55,minRadius=0,maxRadius=0)
    #     if circles is not None:
    #        print('in circle detection')
    #        circles = np.round(circles[0, :]).astype("int")
    #        for (x,y,r) in circles:
                     
    #            cv2.circle(im0, (x,y), r, (36,255,12), 3)  
...
#            txt_path = str(save_dir / 'labels' / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # img.txt
          ...
          #  print(vertics_rec_array)
           # print(np.shape(vertics_rec_array))
            ...
            for i in range(0,m[0]):
             ...
             # print('round(vertics_rec_array[i][0])',round(vertics_rec_array[i][0]),'round(vertics_rec_array[i][1])',round(vertics_rec_array[i][1]))
            #  print('round(vertics_rec_array[i][2])',round(vertics_rec_array[i][2]),'round(vertics_rec_array[i][3])',round(vertics_rec_array[i][3]))

              #print(im0[round(vertics_rec_array[i][3]):round(vertics_rec_array[i][1]),round(vertics_rec_array[i][0]):round(vertics_rec_array[i][2]),:])
              circle_det_center=detect_circle(im0[round(vertics_rec_array[i][1]):round(vertics_rec_array[i][3]),round(vertics_rec_array[i][0]):round(vertics_rec_array[i][2])])
            
            gears_inform=dict(zip(labels,circle_det_center))
        ...
        #     # convert 2d point to 3d 3d from convert file using  function pointconvert
            ...
            gears_informdict= pointconvert(gears_inform)
            print(gears_informdict)
        #     # gears_informarray=np.array(list(gears_informdict.items()))

        #     # #publish gears information array
           # pub3(gears_informdict)
        # #     # print(gears_informarray)  

        #     radius = 2
        #     color = (255, 0, 0)
        #     thickness = 2
            
        #     for i in gears_inform.values():
        #       x=i[0]
        #       y=i[1]
        #       print('circle')
        #       cv2.circle(im0,(x,y),radius,color,thickness)
        #     for i in centre_circle:
        #       x=i[0]
        #       y=i[1]
        #       print('box')
        #       cv2.circle(im0,(x,y),radius,(0,255,0),thickness)          
...
# def object_list():
#     pub = rospy.Publisher('detect',Int32MultiArray, queue_size=10)
#     rospy.init_node('talker', anonymous=True)
#     rate = rospy.Rate(10) # 10hz
#     x=np.array([['gear1', (512, 428)],['gear5', (51278, 4288)]])
#     msg=Int32MultiArray
#     msg.data=x
#     rospy.loginfo(x)
#     pub.publish(x)
#     rate.sleep()
def pointconvert(gear_dictionary):
        ...
        for i  in gear_dictionary.values():
            color_point = [i[0],i[1]]
           ...
           # print(depth_point_) 
            ...
            if depth_point_[0]<0 or depth_point_[1]<0: 
                 ...
                 dx,dy, dz = rs.rs2_deproject_pixel_to_point(color_intrin, color_point , depth) 

                #  print("depth from point to camera ", depth)
                #  print(f'x is {dx} and y is {dy} and z is {dz}')
                 gears_with_depth_point.append([dx,dy,dz])
        gears_inform=dict(zip(gear_dictionary.keys(),gears_with_depth_point))
...
def talker(data):
    ...
    pub = rospy.Publisher('topic_1', realsense, queue_size=10)
	#rospy.init_node('talker', anonymous=True)
    msg=realsense()
    ...
    for l,i in data.items():
        ...
        if np.sqrt((i[0]-x_prev)**2 +(i[1]-y_prev)**2 )>0.01 and np.sqrt((i[0]-x_prev)**2 +(i[1]-y_prev)**2 )<0.5:

                msg.pose.position.x=i[0]
                msg.pose.position.y=i[1]
                msg.pose.position.z=i[2]
...
def detect_callback(data):
    ...
    for l,i in gears_informdict.items(): 
            print(l)
            if l=="gear":
              response.location.header.frame_id='gear'
              response.location.pose.position.x=i[0]
              response.location.pose.position.y=-i[1]
...
def detect_circle(img):
  ...
  #'method 2' ..
    ...
    #         center = [int(x),int(y)]
    #         print("cen",center)
    #         circle_det_center.append(center)
    #         radius = int(radius)
  ...
  #'method 2' ..
   ...
   # circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT,1,30,param1=50,param2=50,minRadius=0,maxRadius=0)
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT,1,30,param1=60,param2=55,minRadius=0,maxRadius=0)
    if circles is not None:
           print('in circle detection')
           circles = np.round(circles[0, :]).astype("int")
           for (x,y,r) in circles:
